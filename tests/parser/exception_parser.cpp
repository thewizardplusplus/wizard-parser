...
#include "../../source/thewizardplusplus/wizard_parser/parser/exception_parser.hpp"
...
TEST_CASE("parser::exception_parser class", "[parser]") {
	...
	SECTION("without tokens") {
		...
		const auto [ast, rest_tokens] = exception_parser->parse({});
		CHECK(!ast.has_value());

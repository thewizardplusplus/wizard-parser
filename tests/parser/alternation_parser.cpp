...
TEST_CASE("parser::alternation_parser class", "[parser]") {
	...
	SECTION("without matches and with both rests offsets of a same length") {
		auto left_tokens = lexer::token_group{
			{"one", "two", 1},
			{"three", "four", 4}
		};
		auto left_mock_parser = fakeit::Mock<parser::rule_parser>{};
		fakeit::When(Method(left_mock_parser, parse))
			.Return(parser::parsing_result{{}, left_tokens});
		fakeit::Fake(Dtor(left_mock_parser));

		auto right_tokens = lexer::token_group{
			{"five", "six", 1},
			{"seven", "eight", 4}
		};
		auto right_mock_parser = fakeit::Mock<parser::rule_parser>{};
		fakeit::When(Method(right_mock_parser, parse))
			.Return(parser::parsing_result{{}, right_tokens});
		fakeit::Fake(Dtor(right_mock_parser));
		...
		CHECK(!ast.has_value());
		CHECK(rest_tokens == lexer::token_span{left_tokens});
	...
	SECTION("with a right match") {
		...
		const auto [ast, rest_tokens] = alternation_parser->parse(tokens);
		REQUIRE(ast.has_value());
		CHECK(*ast == parser::ast_node{"three", "four", {}, 4});
		CHECK(rest_tokens == lexer::token_span{tokens}.subspan(2));

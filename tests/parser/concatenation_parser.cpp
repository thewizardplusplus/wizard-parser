...
#include "../../source/thewizardplusplus/wizard_parser/parser/concatenation_parser.hpp"
...
TEST_CASE("parser::concatenation_parser class", "[parser]") {
	...
	SECTION("without tokens") {
		...
		const auto [ast, rest_tokens] = concatenation_parser->parse({});
		CHECK(!ast.has_value());
